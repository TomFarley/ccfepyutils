...
def hoverinfo_text(names, values, format='0.3g'):
    ...
    for i in range(0, len(values[0])):
        ...
        d = {k:v[i] if v[i] is not None else np.nan for k, v in dic.iteritems()}
...
def input_output_scatter3d(inp, output, extra_points={}, axis_labels=None, online=True, join_in_out=True,
    ...
    trace1 = go.Scatter3d(x=x1, y=y1, z=z1, mode='markers', text=hoverinfo_text(axis_labels, inp), hoverinfo='text',
                          marker={'size': 3, "line": {"width": 0.5}, 'opacity': 1.0}, name="input")
    trace2 = go.Scatter3d(x=x2, y=y2, z=z2, mode='markers', text=hoverinfo_text(axis_labels, output), hoverinfo='text',
                          marker={'size': 3, "line": {"width": 0.5}, 'opacity': 1.0}, name="output")
    data += [trace1, trace2]
    for key, (xe,ye,ze) in extra_points.iteritems():
        trace_points = go.Scatter3d(x=xe, y=ye, z=ze, mode='markers', text=hoverinfo_text(axis_labels, (xe,ye,ze)),
                                    hoverinfo='text', marker={'size': 4, "line": {"width": 0.5}, 'opacity': 1.0},
...
def make_iterable(obj, cast=None):
